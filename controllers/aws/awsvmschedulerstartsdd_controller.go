...
// AWSVMSchedulerStartsddReconciler reconciles a AWSVMSchedulerStartsdd object
type AWSVMSchedulerStartsddReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsdds,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsdds/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsdds/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStartsdd object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *AWSVMSchedulerStartsddReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrllog.FromContext(ctx)
	//log := r.Log.WithValues("AWSVMScheduler", req.NamespacedName)
	log.Info("Reconciling AWSVMSchedulerStartsdd")
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStartsdd{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStartsdd(awsVMScheduler)
...
func (r *AWSVMSchedulerStartsddReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStartsdd{}).
		Complete(r)
}

func (r *AWSVMSchedulerStartsddReconciler) cronJobForAWSVMSchedulerStartsdd(awsVMScheduler *awsv1.AWSVMSchedulerStartsdd) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStartsddLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
func AWSVMSchedulerStartsddLabels(v *awsv1.AWSVMSchedulerStartsdd, tier string) map[string]string {
	return map[string]string{
		"app":                       "AWSVMSchedulerStartsdd",
		"AWSVMSchedulerStartsdd_cr": v.Name,
		"tier":                      tier,
	}
}
